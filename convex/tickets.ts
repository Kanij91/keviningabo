...
export const createTicket = mutation({
  args: {
    ...
    category: v.union(v.literal("hardware"), v.literal("software"), v.literal("network"), v.literal("account"), v.literal("other")),
  },
  ...
  handler: async (ctx) => {
    ...
    const ticketsWithTechnicians = await Promise.all(
      tickets.map(async (ticket) => {
        let assignedTechnicianName = null;
...
export const getMyTickets = query({
  ...
  handler: async (ctx) => {
    ...
    if (currentUser.role === "end-user") {
      ...
      return await ctx.db
        .query("tickets")
        .withIndex("by_requester_email", (q) => q.eq("requesterEmail", currentUser.email))
        .order("desc")
        .collect();
    } else {
      ...
      return await ctx.db
        .query("tickets")
        .withIndex("by_assigned_technician", (q) => q.eq("assignedTechnician", userId))
        .order("desc")
        .collect();
    }
  },
...
export const getTicketsByRequesterEmail = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.db
      .query("tickets")
      .withIndex("by_requester_email", (q) => q.eq("requesterEmail", args.email))
      .order("desc")
      .collect();
  },
});
...
export const updateTicket = mutation({
  args: {
    ...
    status: v.optional(v.union(v.literal("new"), v.literal("assigned"), v.literal("in-progress"), v.literal("on-hold"), v.literal("resolved"), v.literal("closed"))),
  ...
  handler: async (ctx, args) => {
    ...
    if (args.resolutionNotes !== undefined) updateData.resolutionNotes = args.resolutionNotes;
...
export const searchTickets = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db
      .query("tickets")
      .withSearchIndex("search_tickets", (q) => q.search("title", args.searchTerm))
      .collect();
  },
});
